...
class GetCharactersListUsecase @Inject constructor(
    ...
    operator fun invoke(pageNumber:Int) : Flow<Resource<List<SWCharacter>>> = flow {
        try {
            ...
            val swCharactersFromDB : List<SWCharacter> = dao.getSneakersInCart()
            if(swCharactersFromDB.isEmpty()){
                val characters = repository.getSWCharacters(pageNumber).results
                characters.forEach { dao.insertSWCharacter(it.toSWCharacter()) }
                emit(Resource.Success<List<SWCharacter>>(characters.map { it.toSWCharacter() }))
            } else {
            ...
            emit(Resource.Error<List<SWCharacter>>(e.localizedMessage ?: "Unexpected Error Occurred"))
        }
    }
}